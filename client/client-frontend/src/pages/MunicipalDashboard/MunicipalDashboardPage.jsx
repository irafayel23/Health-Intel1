...
import PolicyReportsPanel from "./PolicyReportsPanel";
...
export default function MunicipalDashboardPage() {
  ...
  const navItems = [
    { id: "dashboard", label: "Executive Dash", icon: LayoutDashboard },
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      <aside className="w-64 bg-slate-800 text-white flex flex-col flex-shrink-0">
        ...
        <nav className="flex-1 py-4 flex flex-col gap-1">
          ...
          {/* Mayor sticker */}
          <div className="mt-auto mb-4 mx-4 p-4 bg-slate-700/50 border border-slate-600 rounded-lg flex items-center gap-3">
            <div className="bg-slate-600 p-2 rounded-full text-sky-300">
              <Globe className="w-5 h-5" />
            </div>
            <div>
              <p className="font-bold text-sm">Municipal Mayor</p>
              <p className="text-xs text-slate-400">Elected Official</p>
            </div>
          </div>
        </nav>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-gray-200 px-6 py-4">
          <h1 className="text-lg font-semibold text-gray-900">
            Municipal Official Portal — Executive Monitoring
          </h1>
        </header>
        <main className="flex-1 overflow-y-auto p-6">
          {activeView === "dashboard" && <ExecutiveDashboardPanel />}
          {activeView === "heatmap" && <HeatmapPanel />}
          {activeView === "reports" && <PolicyReportsPanel />}
        </main>
      </div>
    </div>
  );
